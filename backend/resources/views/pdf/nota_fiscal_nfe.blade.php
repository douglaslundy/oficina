...
@php
  // Normaliza $nota->itens (Eloquent) pro mesmo formato de array que
  // pdf/danfe.blade.php já usa (extraído do XML por DanfeRenderer) — o
  // corpo compartilhado (partials/danfe_corpo) não sabe nem precisa saber
  // de onde cada caminho veio.
  $itensNormalizados = $nota->itens->map(fn ($i) => [
    'codigo'         => $i->sku ?: $i->produto_id,
