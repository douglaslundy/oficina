...
class OrdemServicoNumeracaoTest extends TestCase
{
    ...
    private function criarOficinaComAdmin(string $slug, string $cnpj, string $email, string $cpfAdmin, string $cpfCliente): array
    {
        $oficina = Oficina::create([
            'nome' => "Oficina {$slug}", 'slug' => $slug, 'cnpj' => $cnpj, 'status' => 'ATIVA',
        ]);
    ...
    {
        [$oficinaA, $tokenA, $clienteA] = $this->criarOficinaComAdmin('oficina-num-a', '11111111111111', 'a@numero-test.com', '11111111111', '22222222222');
